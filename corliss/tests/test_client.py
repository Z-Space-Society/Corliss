...
class ResolutionTests(TestCase):
    def test_did_passes_through(self):
        self.assertEqual(
            atproto.resolve_handle_to_did("did:plc:abc"), "did:plc:abc"
        )
    ...
    def test_handle_resolves_via_wellknown(self, mock_get):
        ...
        self.assertEqual(
            atproto.resolve_handle_to_did("alice.bsky.social"), "did:plc:xyz"
        )
    ...
    def test_handle_resolver_fallback(self, mock_get):
        ...
        self.assertEqual(
            atproto.resolve_handle_to_did("alice.example.com"), "did:plc:fallback"
        )
    ...
    def test_pds_endpoint_from_doc(self):
        ...
        self.assertEqual(
            atproto.pds_endpoint_from_doc(doc), "https://pds.example.com"
        )

    def test_pds_endpoint_missing_raises(self):
        with self.assertRaises(atproto.OAuthError):
            atproto.pds_endpoint_from_doc({"service": []})
    ...
    def test_discover_auth_server(self, mock_get):
        ...
        meta = atproto.discover_auth_server("https://pds.example.com")
...
class EmailSourcingTests(TestCase):
    ...
    def test_fetch_session_email_success(self, mock_get):
        ...
        email, confirmed = atproto.fetch_session_email(
            "https://pds.example.com", "AT", dpop_key=atproto.generate_key()
        )
...
class PkceDpopTests(TestCase):
    ...
    def test_dpop_proof_structure_and_signature(self):
        key = atproto.generate_key()
        proof = atproto.make_proof(
...
class ClientAssertionAndFlowTests(TestCase):
    ...
    def test_client_assertion_claims(self):
        assertion = atproto.build_client_assertion("https://auth.example")
        ...
        self.assertEqual(payload["iss"], atproto.client_id())
        self.assertEqual(payload["sub"], atproto.client_id())

    @patch("corliss.atproto.requests.post")
    ...
    def test_exchange_code_returns_token(self, mock_post):
        ...
        token, nonce = atproto.exchange_code(
            meta, code="code", code_verifier="v", dpop_key=atproto.generate_key()
        )
