...
or `did:web:`). The password is read from CORLISS_ADMIN_PASSWORD (never a
CLI arg — that would leave it in the process list) and is only ever set at
creation:
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **opts):
        ...
        with transaction.atomic():
            ...
            if created:
                password = os.environ.get("CORLISS_ADMIN_PASSWORD")
                if not password:
                    raise CommandError(
                        "CORLISS_ADMIN_PASSWORD is required to create the admin user"
                    )
